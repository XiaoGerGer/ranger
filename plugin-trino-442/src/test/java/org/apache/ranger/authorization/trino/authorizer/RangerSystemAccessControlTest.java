...
public class RangerSystemAccessControlTest {
  ...
  private static final Collection<Identity> queryOwners = ImmutableSet.of(bob, alice, Identity.ofUser("frank"));
  ...
  private static final CatalogSchemaRoutineName catalogSchemaRoutineName = new CatalogSchemaRoutineName("alice-catalog", "schema", "function");
  ...
  public void testMisc()
  {
    ...
    List<ViewExpression> retRowFilters  = accessControlManager.getRowFilters(context(alice), aliceTable);
    assertTrue(retRowFilters.isEmpty());

    accessControlManager.canExecuteFunction(context(alice), catalogSchemaRoutineName);
  ...
  private SystemSecurityContext context(Identity id) {
    return new SystemSecurityContext(id, QueryId.valueOf("123"), Instant.now());
  }
}